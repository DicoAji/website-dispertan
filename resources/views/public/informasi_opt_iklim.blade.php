...
@section('title', 'Informasi OPT dan Perkiraan Iklim')

@section('content')
    <section class="pt-12">
        <div class="container mx-auto px-4 max-w-7xl space-y-8">
            <div class="text-center">
                <h2 class="text-2xl font-bold text-gray-900 mb-1">Informasi OPT dan Perkiraan Iklim</h2>
                <p class="text-green-600 italic">{{ $profile->nama_opd ?? 'Dinas Pertanian Kabupaten Grobogan' }}</p>
            </div>

            @php
                // Memanggil data langsung dari database untuk ID 9 (Informasi OPT dan Perkiraan Iklim)
                $dokumen = \App\Models\FileDinas::find(9);
            @endphp
            ...
            <div class="p-4 md:p-8 shadow-lg border rounded-lg bg-white mt-8">
                @if ($dokumen && $dokumen->file)
                    ...
                    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between border-b pb-4 gap-4">
                        <h2 class="text-md text-gray-700 flex items-center font-bold">
                            ...
                            {{ $dokumen->uraian ?? 'Dokumen Informasi OPT dan Perkiraan Iklim' }}
                        </h2>
                        <a href="{{ asset('storage/dokumen/' . $dokumen->file) }}" download
                ...
                @else
                    ...
                    <div class="text-center py-20 text-gray-400">
                        ...
                        <p class="text-lg">Dokumen Informasi OPT dan Perkiraan Iklim belum tersedia.</p>
                    </div>
                @endif
            </div>
