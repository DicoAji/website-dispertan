...
<html lang="id">
  <head>
    ...
    <style>
      #sidebar, #main-content, #toggleIcon { transition: all 0.3s ease-in-out; }
    </style>
  </head>
  <body class="bg-gray-100 font-sans overflow-hidden">
    <div class="flex h-screen">
      <div id="sidebar" class="fixed inset-y-0 left-0 z-30 w-64 bg-green-800 text-white transform translate-x-0 shadow-xl">
        ...
        <nav class="mt-6 px-4 space-y-2">
            ...
            <a href="{{ route('pegawai.index') }}" class="flex items-center py-3 px-4 rounded-lg {{ request()->routeIs('pegawai.*') ? 'bg-green-900 shadow-inner' : 'hover:bg-green-700' }} transition">
                <i class="fas fa-users w-6"></i> Data Pegawai
            </a>

            <a href="{{ route('berita.index') }}" class="flex items-center py-3 px-4 rounded-lg {{ request()->routeIs('berita.*') ? 'bg-green-900' : 'hover:bg-green-700' }}">
                <i class="fas fa-newspaper w-6"></i> Berita
            </a>

            <a href="{{ route('profile.index') }}" class="flex items-center py-3 px-4 rounded-lg {{ request()->routeIs('profile.*') ? 'bg-green-900 shadow-inner' : 'hover:bg-green-700' }} transition">
      ...
      <div id="main-content" class="flex-1 flex flex-col min-w-0 ml-64">
        <header class="bg-white shadow-sm py-4 px-6 flex justify-between items-center border-b border-green-600">
          <div class="flex items-center">
            <button id="menuBtn" class="p-2 rounded-md text-gray-600 hover:bg-gray-100 focus:outline-none z-40">
          ...
          <div class="flex items-center space-x-4">
            ...
            <img class="h-10 w-10 rounded-full border-2 border-green-500 shadow-sm" src="https://ui-avatars.com/api/?name=Admin&background=059669&color=fff" />
          </div>
        </header>
    ...
    <script>
      ...
      menuBtn.addEventListener("click", () => {
        ...
        if (window.innerWidth >= 768) { mainContent.classList.toggle("ml-64"); }
